...
#ifdef CONFIG_HUAWEI_KERNEL
	else if (!strncmp(pdev->name, smd_ch_name[17],
				strnlen(smd_ch_name[17], SMD_MAX_CH_NAME_LEN)))
...
static int __init smd_tty_init(void)
{
	...
	spin_lock_init(&smd_tty[17].reset_lock);
	smd_tty[17].is_open = 0;
	init_waitqueue_head(&smd_tty[17].ch_opened_wait_queue);
