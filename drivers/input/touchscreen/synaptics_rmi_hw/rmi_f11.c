...
static struct tp_resolution_conversion lcd_resolution;
static uint16_t check_scope_x(uint16_t x)
{
	uint16_t temp = x;
	if (x >= lcd_resolution.lcd_x -1)
	{
		temp = lcd_resolution.lcd_x -2;
	}
	if (x <= 1)
	{
		temp = 1;
					...
					u8 finger_state, u8 n_finger)
{
	...
	int x, y, z;
	int w_x, w_y, w_max, w_min, orient,x_lcd,y_lcd_all;
	int temp;

	x_lcd = lcd_resolution.lcd_x;
	y_lcd_all = lcd_resolution.lcd_all;
	...
	if (prev_state && !finger_state) {
		...
		if (machine_is_msm7x27a_U8661())
		{
			...
			/*for solving virtual keys sensitive*/
			w_max = z;
		}
		if (axis_align->flip_y)
...
static void f11_set_abs_params(struct rmi_function_container *fc, int index)
{
	...
	int x_min, x_max, y_min, y_max,x_lcd,y_lcd;

	x_lcd = lcd_resolution.lcd_x;
	y_lcd = lcd_resolution.lcd_y;
...
static int rmi_f11_initialize(struct rmi_function_container *fc)
{
	...
	dev_info(&fc->dev, "Initializing F11 values for %s.\n",
		 pdata->sensor_name);
	...
	*/
	...
	fc->data = f11;
	pdata->get_phone_version(&lcd_resolution);
#if	RESUME_REZERO
...
static int rmi_f11_register_devices(struct rmi_function_container *fc)
{
	...
	for (i = 0; i < (f11->dev_query.nbr_of_sensors + 1); i++) {
		...
		* phys name for input device */
		...
		input_dev->name = "synaptics";//f11->sensors[i].input_name;
		...
		dev_info(&fc->dev, "%s: Sensor %d hasRel %d.\n",
		...
		if (f11->sensors[i].sens_query.has_gestures) {
			...
			if (!vm_pdata) {
				dev_err(&fc->dev, "Failed to"
					" get the pdata virtualbutton map.\n");
				goto error_unregister;
			}
			...
			if (!vm_sensor->map) {
				dev_err(&fc->dev, "Failed to"
					" allocate the virtualbutton map.\n");
				rc = -ENOMEM;
				goto error_unregister;
			}
