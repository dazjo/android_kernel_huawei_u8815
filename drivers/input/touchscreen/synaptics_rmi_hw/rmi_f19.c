	...
	struct f19_data *f19)
{
	...
	u8 ctrl_base_addr = f19->general_control_address;
	...
	if (button_control->int_enabled_button) {
		error = rmi_read_block(rmi_dev, ctrl_base_addr,
			...
			(sizeof(u8)*f19->button_register_count));
		if (error < 0) {
			dev_err(&rmi_dev->dev,
...
static int rmi_f19_alloc_memory(struct rmi_function_container *fc)
{
	...
	f19->button_register_count = (f19->button_count + 7) / 8;
	f19->button_data_buffer =
	    kcalloc(f19->button_register_count,
	...
	f19->button_control.int_enabled_button =
		kzalloc(f19->button_register_count*sizeof(u8), GFP_KERNEL);
...
static int rmi_f19_config(struct rmi_function_container *fc)
{
	...
	retval = rmi_write_block(fc->rmi_dev, data->int_enable_button_address,
		(u8 *)data->button_control.int_enabled_button,
	...
	retval = rmi_write_block(fc->rmi_dev, data->single_button_address,
		(u8 *)data->button_control.single_button,
		sizeof(u8)*data->button_register_count);
	if (retval < 0) {
		dev_err(&fc->dev,
					 ...
					 char *buf)
{
	...
	for (i = 0; i < data->button_count; i++) {
		...
		single_button = ((data->button_control.
			single_button[button_reg] >> button_shift) & 0x01);
					 ...
					 const char *buf, size_t count)
{
	...
	len = rmi_write_block(fc->rmi_dev, data->all_button_threshold_address,
		(u8 *)&(data->button_control.all_button_hysteresis_threshold),
			sizeof(struct f19_0d_control_6));
	if (len < 0) {
