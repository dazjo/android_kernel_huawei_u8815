...
static int product_matches_pid_to_functions(u16 pid)
{
    ...
    char **functions;
    int i;
    /* disabled: 1 presents disable and 0 presents enalbe */
    int disabled;
    ...
    list_for_each_entry(f, &android_config_driver.functions, list)
    {
        ...
        for (i=0; i<count; i++)
        {
            if (!strcmp(f->name, *(functions+i)))
            {
                ...
                /* Enable the function if it matches the pid*/
                disabled = 0;
        ...
        /* If the function's disabled/enabled status should be changed,
         * set the right status and send the uevent.
         */
        if (!!f->disabled != disabled)
        {
            usb_function_set_enabled(f, !disabled);            
            USB_PR("function: %s has changed to %d.\n",f->name,!disabled);
        }
    }
...
{  
    ...
    if (product_id == pid)
    {
        USB_PR("%s, pid=0x%x is same, ignore it\n", __func__, pid);
        usb_switch_para.inprogress = 0;
        return 0;
    }
