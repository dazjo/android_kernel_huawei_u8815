...
static int adp5587_chip_reset(void)
{
	...
	gpio_config = GPIO_CFG(ADP5587_RESET_GPIO, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA);
...
static int adp5587_setup_irq(struct i2c_client *client)
{
	...
	gpio_config = GPIO_CFG(ADP5587_INT_GPIO, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA);
...
static int adp5587_input_device_init(struct i2c_client *client)
{
	...
	if (!client_data->input_dev) {
		...
		client_data->keymap = keypad_keymap_7x27a;
		keynum = sizeof(keypad_keymap_7x27a)/sizeof(keypad_keymap_7x27a[0]);
...
static int adp5587_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	uint8_t device_id;
 	...
 	{
 		printk(KERN_ERR "project isn't support qwerty \n");		
		rc = -ENODEV;		
		return  rc;
 	}
	...
	if(device_id < 0){			
		printk(KERN_ERR "adp5587_chip_init=%d \n",rc);
		goto exit_chip_init_failed;
	}
